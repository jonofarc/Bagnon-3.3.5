...
local FrameOptions = Bagnon.OptionsPanel:New('BagnonOptions_Frame', 'Bagnon',
                                             L.FrameSettings,
                                             L.FrameSettingsTitle)
FrameOptions:Hide()
...
function FrameOptions:FRAME_LAYER_UPDATE(msg, frameID, layer)
    if self:GetFrameID() == frameID then self:GetLayerSlider():UpdateValue() end
end

function FrameOptions:FRAME_SCALE_UPDATE(msg, frameID, scale)
    if self:GetFrameID() == frameID then self:GetScaleSlider():UpdateValue() end
end
...
function FrameOptions:OnHide() self:UpdateMessages() end
...
function FrameOptions:AddWidgets()
    ...
    toggleMoneyFrame:SetPoint('TOPLEFT', toggleBagFrame, 'BOTTOMLEFT', 0,
                              -CHECK_BUTTON_SPACING)

    local toggleDBOFrame = self:CreateToggleDBOFrameCheckbox()
    toggleDBOFrame:SetPoint('TOPLEFT', toggleMoneyFrame, 'BOTTOMLEFT', 0,
                            -CHECK_BUTTON_SPACING)

    local sortButtonFrame = self:CreateSortButtonCheckbox()
    sortButtonFrame:SetPoint('TOPLEFT', toggleDBOFrame, 'BOTTOMLEFT', 0,
                             -CHECK_BUTTON_SPACING)

    local toggleSearchFrame = self:CreateToggleSearchFrameCheckbox()
    toggleSearchFrame:SetPoint('TOPLEFT', sortButtonFrame, 'BOTTOMLEFT', 0,
                               -CHECK_BUTTON_SPACING)

    local toggleOptionsFrame = self:CreateToggleOptionsCheckbox()
    toggleOptionsFrame:SetPoint('TOPLEFT', toggleSearchFrame, 'BOTTOMLEFT', 0,
                                -CHECK_BUTTON_SPACING)

    local reverseSlotOrdering = self:CreateReverseSlotOrderCheckbox()
    reverseSlotOrdering:SetPoint('TOPLEFT', toggleOptionsFrame, 'BOTTOMLEFT', 0,
                                 -CHECK_BUTTON_SPACING)

    local bagBreak = self:CreateBagBreakCheckbox()
    bagBreak:SetPoint('TOPLEFT', reverseSlotOrdering, 'BOTTOMLEFT', 0,
                      -CHECK_BUTTON_SPACING)
    ...
    -- non-cleanable slots
    local nonCleanableSlots = self:CreateNonCleanableSlotsSlider()
    nonCleanableSlots:SetWidth(180)
    nonCleanableSlots:SetPoint('TOPLEFT', bagBreak, 'BOTTOMLEFT', 0, -12)
...
function FrameOptions:UpdateWidgets()
    ...
    self:GetToggleBagFrameCheckbox():UpdateChecked()
    self:GetToggleBagFrameCheckbox():SetDisabled(
        self:GetFrameID() == 'keys' or self:GetFrameID() == 'guildbank')
    ...
    self:GetReverseSlotOrderCheckbox():UpdateChecked()
    self:GetReverseSlotOrderCheckbox():SetDisabled(
        self:GetFrameID() == 'guildbank')

    self:GetBagBreakCheckbox():UpdateChecked()
    self:GetBagBreakCheckbox():SetDisabled(
        self:GetFrameID() == 'keys' or self:GetFrameID() == 'guildbank')

    self:GetNonCleanableSlotsSlider():UpdateValue()
...
function FrameOptions:GetFrameSelector() return self.frameSelector end
...
function FrameOptions:GetColorSelector() return self.colorSelector end

-- background color
function FrameOptions:CreateBorderColorSelector()
    local selector = Bagnon.OptionsColorSelector:New(L.FrameBorderColor, self,
                                                     true)
...
function FrameOptions:GetBorderColorSelector() return self.borderColorSelector end
...
function FrameOptions:GetColumnsSlider() return self.columnsSlider end
...
function FrameOptions:GetSpacingSlider() return self.spacingSlider end
...
function FrameOptions:CreateScaleSlider()
    ...
    slider.GetFormattedText = function(self, value) return value .. '%' end
...
function FrameOptions:GetScaleSlider() return self.scaleSlider end
...
function FrameOptions:GetOpacitySlider() return self.opacitySlider end

-- layer
function FrameOptions:CreateLayerSlider()
    local availableLayers = self:GetSettings():GetAvailableLayers()
    local slider = Bagnon.OptionsSlider:New(L.FrameLayer, self, 1,
                                            #availableLayers, 1)
    ...
    slider.GetSavedValue = function(self)
        ...
        for k, v in pairs(self.layers) do if v == layer then return k end end
        return 1
    end

    slider.GetFormattedText =
        function(self, value) return self.layers[value] end
...
function FrameOptions:GetLayerSlider() return self.layerSlider end

-- non-cleanable slots
function FrameOptions:CreateNonCleanableSlotsSlider()
    local slider = Bagnon.OptionsSlider:New('Non-Cleanable Slots', self, 0, 25,
                                            1)

    slider.SetSavedValue = function(self, value)
        self:GetParent():GetSettings():SetNonCleanableSlots(value)
    end

    slider.GetSavedValue = function(self)
        return self:GetParent():GetSettings():GetNonCleanableSlots()
    end

    slider.GetFormattedText = function(self, value) return value end

    self.nonCleanableSlotsSlider = slider
    return slider

end

function FrameOptions:GetNonCleanableSlotsSlider()
    return self.nonCleanableSlotsSlider
end
...
function FrameOptions:GetSortButtonCheckbox() return self.sortButtonCheckbox end
...
function FrameOptions:GetBagBreakCheckbox() return self.bagBreakCheckbox end
...
function FrameOptions:GetFrameID() return self.frameID end
