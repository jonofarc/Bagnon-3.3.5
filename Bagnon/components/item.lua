...
function ItemSlot:Create()
    ...
    local item = self:Bind(self:GetBlizzardItemSlot(id) or
                               self:ConstructNewItemSlot(id))
    item:Hide()
    ...
    -- red background for non-cleanable slots
    local nonCleanableBackground = item:CreateTexture(nil, 'ARTWORK')
    nonCleanableBackground:SetPoint('TOPLEFT', item, 'TOPLEFT', 1, -1)
    nonCleanableBackground:SetPoint('BOTTOMRIGHT', item, 'BOTTOMRIGHT', -1, 1)
    -- nonCleanableBackground:SetTexture(1, 0, 0, 0.15) // jon
    nonCleanableBackground:Hide()
    item.nonCleanableBackground = nonCleanableBackground
...
function ItemSlot:ConstructNewItemSlot(id)
    return CreateFrame('Button', 'BagnonItemSlot' .. id, nil,
...
function ItemSlot:GetBlizzardItemSlot(id)
    ...
    if not self:CanReuseBlizzardBagSlots() then return nil end
...
function ItemSlot:CanReuseBlizzardBagSlots()
    return Bagnon.Settings:AreAllFramesEnabled() and
               (not Bagnon.Settings:IsBlizzardBagPassThroughEnabled())
end

-- returns an available item slot
...
function ItemSlot:ITEM_SLOT_UPDATE(msg, bag, slot) self:Update() end

function ItemSlot:ITEM_LOCK_CHANGED(event, bag, slot) self:UpdateLocked() end

function ItemSlot:ITEM_SLOT_UPDATE_COOLDOWN(msg, bag, slot) self:UpdateCooldown() end

function ItemSlot:TEXT_SEARCH_UPDATE(msg, frameID, search) self:UpdateSearch() end

function ItemSlot:BAG_SEARCH_UPDATE(msg, frameID, search)
    if self:GetFrameID() == frameID then self:UpdateBagSearch() end
end

function ItemSlot:ITEM_HIGHLIGHT_QUALITY_UPDATE(msg, enable) self:UpdateBorder() end

function ItemSlot:ITEM_HIGHLIGHT_QUEST_UPDATE(msg, enable) self:UpdateBorder() end

function ItemSlot:ITEM_HIGHLIGHT_OPACITY_UPDATE(msg, opacity) self:UpdateBorder() end

function ItemSlot:SHOW_EMPTY_ITEM_SLOT_TEXTURE_UPDATE(msg, enable) self:Update() end

function ItemSlot:ITEM_SLOT_COLOR_ENABLED_UPDATE(msg, type, r, g, b)
    self:Update()
end

function ItemSlot:ITEM_SLOT_COLOR_UPDATE(msg, type, r, g, b) self:Update() end

function ItemSlot:NON_CLEANABLE_SLOTS_UPDATE(msg, slots) self:Update() end

function ItemSlot:QUEST_ACCEPTED() self:UpdateBorder() end

function ItemSlot:UNIT_QUEST_LOG_CHANGED() self:UpdateBorder() end

function ItemSlot:HandleEvent(msg, ...)
    local action = self[msg]

    if action then action(self, msg, ...) end
end

--[[ Frame Events ]] --

function ItemSlot:OnShow() self:Update() end

function ItemSlot:OnHide() self:HideStackSplitFrame() end

function ItemSlot:OnDragStart()
    if self:IsCached() and CursorHasItemSlot() then ClearCursor() end
end
    ...
    if self:IsCached() then
        ...
        if self:IsBank() then
            if self:GetItem() then
                self:AnchorTooltip()
                GameTooltip:SetInventoryItem('player', BankButtonIDToInvSlotID(
                                                 self:GetID()))
                GameTooltip:Show()
                CursorUpdate(self)
            end
...
function ItemSlot:Update()
    ...
    self:UpdateNonCleanableBackground()
    self:UpdateSearch()
    self:UpdateBagSearch()

    if GameTooltip:IsOwned(self) then self:UpdateTooltip() end
end

-- item link
function ItemSlot:SetItem(itemLink) self.hasItem = itemLink or nil end

function ItemSlot:GetItem() return self.hasItem end
...
function ItemSlot:UpdateSlotColor()
    ...
    -- preserve Bagnon's special bag slot coloring
...
-- item count
function ItemSlot:SetCount(count) SetItemButtonCount(self, count) end

-- readable status
function ItemSlot:SetReadable(readable) self.readable = readable end

-- locked status
function ItemSlot:SetLocked(locked) SetItemButtonDesaturated(self, locked) end

-- locked status
function ItemSlot:UpdateLocked() self:SetLocked(self:IsLocked()) end

-- returns true if the slot is locked, and false otherwise
function ItemSlot:IsLocked()
    return Bagnon.ItemSlotInfo:IsLocked(self:GetPlayer(), self:GetBag(),
                                        self:GetID())
end

--[[ Non-Cleanable Slots ]] --

-- returns the number of protected/non-cleanable slots
function ItemSlot:GetNonCleanableSlots()
    local settings = self:GetSettings()

    if settings and settings.GetNonCleanableSlots then
        return settings:GetNonCleanableSlots() or 0
    end

    return 0
end

-- returns true if this item slot is one of the protected slots
-- uses the same flattened visible-slot order as sorting.lua
function ItemSlot:IsNonCleanableSlot()
    local nonCleanableSlots = self:GetNonCleanableSlots()

    if nonCleanableSlots <= 0 then return false end

    local frame = self:GetParent()

    -- The immediate parent is a dummy bag, so get the actual Bagnon frame.
    if frame and frame.GetParent then frame = frame:GetParent() end

    if not frame or not frame.GetVisibleBags or not frame.GetBagSize then
        return false
    end

    local currentBag = self:GetBag()
    local currentSlot = self:GetID()

    local totalSpaces = 0
    local currentIndex = nil

    for _, bag in frame:GetVisibleBags() do
        local bagSize = frame:GetBagSize(bag)

        for slot = 1, bagSize do
            totalSpaces = totalSpaces + 1

            if bag == currentBag and slot == currentSlot then
    ...
    local firstProtectedIndex = totalSpaces - nonCleanableSlots + 1

    if firstProtectedIndex < 1 then firstProtectedIndex = 1 end

    return currentIndex >= firstProtectedIndex
end

-- updates the red background of non-cleanable slots
function ItemSlot:UpdateNonCleanableBackground()
    local background = self.nonCleanableBackground
...
-- colors the item border based on the quality of the item.
-- hides it for common/poor items
...
function ItemSlot:GetItemSearch() return Bagnon.Settings:GetTextSearch() end
...
function ItemSlot:GetBagSearch() return self:GetSettings():GetBagSearch() end
...
function ItemSlot:GetFrameID() return self.frameID end

function ItemSlot:GetSettings()
    return Bagnon.FrameSettings:Get(self:GetFrameID())
end

function ItemSlot:GetPlayer() return self:GetSettings():GetPlayerFilter() end
...
function ItemSlot:IsBank() return Bagnon.BagSlotInfo:IsBank(self:GetBag()) end
...
function ItemSlot:AtBank() return Bagnon.PlayerInfo:AtBank() end

function ItemSlot:GetItemSlotInfo()
    local texture, count, locked, quality, readable, lootable, link =
        Bagnon.ItemSlotInfo:GetItemInfo(self:GetPlayer(), self:GetBag(),
                                        self:GetID())
...
local QUEST_ITEM_SEARCH = string.format('t:%s|%s',
    ...
    function ItemSlot:IsQuestItem()
        ...
        if self:IsCached() then
            ...
            local isQuestItem, questID, isActive =
                GetContainerItemQuestInfo(self:GetBag(), self:GetID())
...
function ItemSlot:ColoringBagSlots() return Bagnon.Settings:ColoringBagSlots() end
