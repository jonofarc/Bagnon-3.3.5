...
FrameSettings.mt = {__index = FrameSettings}

FrameSettings.objects = setmetatable({}, {
...
function FrameSettings:Get(id) return self.objects[id] end

function FrameSettings:GetID() return self.frameID end
...
function FrameSettings:Show()
    ...
    if not wasShown then self:SendMessage('FRAME_SHOW') end
...
function FrameSettings:IsShown() return (self.shown or 0) > 0 end
...
function FrameSettings:GetScale() return self:GetDB():GetScale() end
...
function FrameSettings:GetOpacity() return self:GetDB():GetOpacity() end
...
function FrameSettings:GetColor() return self:GetDB():GetColor() end
...
function FrameSettings:GetBorderColor() return self:GetDB():GetBorderColor() end
...
function FrameSettings:GetLayer() return self:GetDB():GetLayer() end

function FrameSettings:GetAvailableLayers()
    if not FrameSettings.availableFrameLayers then
        FrameSettings.availableFrameLayers = {
            'LOW', 'MEDIUMLOW', 'MEDIUM', 'MEDIUMHIGH', 'HIGH', 'TOPLEVEL'
        }
    end
...
function FrameSettings:HasBagFrame() return self:GetDB():HasBagFrame() end
...
function FrameSettings:HasMoneyFrame() return self:GetDB():HasMoneyFrame() end
...
function FrameSettings:HasDBOFrame() return self:GetDB():HasDBOFrame() end
...
function FrameSettings:HasSearchToggle() return self:GetDB():HasSearchToggle() end
...
function FrameSettings:HasSortButton() return self:GetDB():HasSortButton() end
...
function FrameSettings:HasOptionsToggle() return self:GetDB():HasOptionsToggle() end
...
function FrameSettings:SetBrokerDisplayObject(objectName)
    if self:GetBrokerDisplayObject() ~= objectName then
        ...
        self:SendMessage('DATABROKER_OBJECT_UPDATE',
                         self:GetBrokerDisplayObject())
    end
end
...
function FrameSettings:IsBagFrameShown() return self.showBagFrame end
...
function FrameSettings:HasBagSlot(slot)
    for i, bagSlot in self:GetBagSlots() do
        if bagSlot == slot then return true end
    end
    return false
end

function FrameSettings:GetBagSlots() return ipairs(self:GetDB():GetBags()) end
...
local function reverseVisibleSlotIterator(obj, i)
    ...
    for j = nextSlot, 1, -1 do
        ...
        if not obj:IsBagSlotHidden(slot) then return j, slot end
    end
...
-- Non-Cleanable Slots
function FrameSettings:SetNonCleanableSlots(slots)
    slots = tonumber(slots) or 0

    if slots < 0 then
        slots = 0
    elseif slots > 25 then
        slots = 25
    end

    if self:GetNonCleanableSlots() ~= slots then
        self:GetDB():SetNonCleanableSlots(slots)
        self:SendMessage('NON_CLEANABLE_SLOTS_UPDATE',
                         self:GetNonCleanableSlots())
    end

end

function FrameSettings:GetNonCleanableSlots()
    return self:GetDB():GetNonCleanableSlots()
end
...
function FrameSettings:IsTextSearchEnabled() return self.enableTextSearch end
...
function FrameSettings:GetBagSearch() return self.bagSearch or false end
