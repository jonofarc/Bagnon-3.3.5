...
-- Protected/non-cleanable slots are never used as a source or target.
for k, target in pairs(spaces) do
local item = target.item


if target.cleanable and item.id and stackable(item) then
  for j = k + 1, #spaces do
    local from = spaces[j]
    local other = from.item

    if from.cleanable and item.id == other.id and stackable(other) then
...
local order, familySpaces = self:GetOrder(spaces, family)
local n = min(#order, #familySpaces)


for index = 1, n do
  local goal = familySpaces[index]
  ...
  if item.space ~= goal then
    ...
    for j = index, n do
      ...
      if other.id == item.id and other.count == item.count then
        local d = moveDistance(other, familySpaces[j])
...
function Sort:GetNonCleanableSlots()
...
if self.itemFrame.GetSettings then
frameSettings = self.itemFrame:GetSettings()
end

if frameSettings and frameSettings.GetNonCleanableSlots then
return frameSettings:GetNonCleanableSlots() or 0
end

if self.itemFrame.GetFrameID then
frameSettings = Bagnon.FrameSettings:Get(self.itemFrame:GetFrameID())
...
if self.itemFrame.GetID then
frameSettings = Bagnon.FrameSettings:Get(self.itemFrame:GetID())
...
function Sort:IsCleanableSpace(space)
return space and space.cleanable
end
...
if space.cleanable and item.id and not item.sorted and self:FitsIn(item.id, family) then
  tinsert(order, space.item)
end

if space.cleanable and space.family == family then
  tinsert(slots, space)
end
...
-- Never move anything into or out of a non-cleanable slot.
if not from.cleanable or not to.cleanable then
return false
end
